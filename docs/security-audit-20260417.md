...
- [x] M-1: CSP `svelte.config.js` `csp.mode='hash'` 도입 — `script-src unsafe-inline` 제거
- [x] M-3: SAML SSO rate limit 추가 (IP당 30회/분)
- [x] M-4: end-session `id_token_hint` RS256 서명 검증 및 sub 일치 확인
- [x] L-1: PBKDF2 → argon2id 전환 (`@hicaru/argon2-pure.js`, 레거시 자동 업그레이드)
- [x] L-3: Fixed Window → Sliding Window(두 버킷 근사) 전환
