...
const DB_ID = isPreview ? process.env.CLOUDFLARE_D1_PREVIEW_DATABASE_ID : process.env.CLOUDFLARE_D1_DATABASE_ID;

const missing: string[] = [];
if (!ACCOUNT_ID) missing.push("CLOUDFLARE_ACCOUNT_ID");
if (!API_TOKEN) missing.push("CLOUDFLARE_API_TOKEN");
if (!DB_ID) missing.push(isPreview ? "CLOUDFLARE_D1_PREVIEW_DATABASE_ID" : "CLOUDFLARE_D1_DATABASE_ID");
...
const applied = new Set((await query("SELECT name FROM _seed_migrations ORDER BY id")).map((r) => r.name));
