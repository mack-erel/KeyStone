...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        ...
        <span class="text-sm text-gray-500"
            >{data.events.length}건 표시 (페이지 {data.pageSize}건)</span>
    </div>
    ...
    <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
        <div class="flex flex-wrap items-end gap-3">
            <div>
                <label for="f-kind" class="block text-xs font-medium text-gray-600"
                    >이벤트 종류</label>
                <select
                    id="f-kind"
                ...
                {:else}
                    {#each data.events as ev (ev.id)}
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3">
                                <span
                                    class="rounded-full px-2 py-0.5 text-xs font-medium {ev.outcome ===
                                    'success'
                            ...
                            <td class="px-4 py-3 font-mono text-xs text-gray-400"
                                >{ev.ip ?? "—"}</td>
                            <td
                                class="max-w-70 truncate px-4 py-3 font-mono text-xs text-gray-500"
                                title={ev.detailJson ?? ""}>
