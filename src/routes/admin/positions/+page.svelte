	...
	<div class="overflow-x-auto rounded-xl border border-gray-200 bg-white shadow-sm">
		<table class="min-w-full divide-y divide-gray-200">
			<thead class="bg-gray-50">
				<tr>
					<th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">직급명</th>
					<th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">코드</th>
					<th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">레벨</th>
						...
						<tr class="hover:bg-gray-50">
							{#if editId === pos.id}
								<td colspan="3" class="px-4 py-3">
									<form method="POST" action="?/update" use:enhance={() => ({ result, update }) => { update(); if (result.type === 'success') editId = null; }} class="flex gap-2 items-center flex-wrap">
										<input type="hidden" name="id" value={pos.id} />
										<input type="text" name="name" value={pos.name} required class="rounded border border-gray-300 px-2 py-1 text-sm focus:outline-none w-32" />
										<input type="text" name="code" value={pos.code ?? ''} placeholder="코드" class="rounded border border-gray-300 px-2 py-1 text-sm focus:outline-none w-24" />
										<input type="number" name="level" value={pos.level} class="rounded border border-gray-300 px-2 py-1 text-sm focus:outline-none w-20" />
