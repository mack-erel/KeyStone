    ...
    {#if showCreate}
        <div class="rounded-xl border border-blue-100 bg-blue-50 p-5">
            ...
            <form
                ...
                <div>
                    <label for="part-name" class="block text-xs font-medium text-gray-700"
                        >파트명 *</label>
                    <input
                        id="part-name"
                        type="text"
                ...
                <div>
                    <label for="part-code" class="block text-xs font-medium text-gray-700"
                        >코드</label>
                    <input
                        id="part-code"
                        type="text"
                ...
                <div>
                    <label for="part-teamId" class="block text-xs font-medium text-gray-700"
                        >소속 팀</label>
                    <select
                        id="part-teamId"
                        name="teamId"
                ...
                <div>
                    <label for="part-description" class="block text-xs font-medium text-gray-700"
                        >설명</label>
                    <input
                        id="part-description"
                        type="text"
                                ...
                                <td colspan="5" class="px-4 py-3">
                                    <form
                                        ...
                                        <select
                                            ...
                                            <option value="">없음 (독립 파트)</option>
                                            {#each data.allTeams as t (t.id)}
                                                <option value={t.id} selected={part.teamId === t.id}
                                                    >{teamLabel(t)}</option>
                                            {/each}
                                        </select>
                                        <select
                                            ...
                                            <option
                                                value="active"
                                                selected={part.status === "active"}>활성</option>
                                            <option
                                                value="inactive"
                                                selected={part.status === "inactive"}
                                                >비활성</option>
                                        </select>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex gap-2">
                                    <button
                                        type="button"
                                        onclick={() =>
                                            (editId = editId === part.id ? null : part.id)}
