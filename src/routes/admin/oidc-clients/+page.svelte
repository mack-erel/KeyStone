...
<div class="space-y-6">
    ...
    {#if newSecret && newClientId}
        <div class="rounded-xl border border-green-200 bg-green-50 p-4">
            ...
            <p class="mb-2 text-xs text-green-700">
                시크릿은 지금만 표시됩니다. 안전한 곳에 저장해 두세요.
            </p>
            <div class="space-y-1">
                <div class="flex items-center gap-2">
                    <span class="w-28 text-xs text-gray-500">Client ID</span>
                    <code class="rounded bg-white px-2 py-1 font-mono text-sm text-gray-800"
                        >{newClientId}</code>
                </div>
                <div class="flex items-center gap-2">
                    <span class="w-28 text-xs text-gray-500">Client Secret</span>
                    <code
                        class="rounded bg-white px-2 py-1 font-mono text-sm break-all text-gray-800"
                        >{newSecret}</code>
                </div>
            </div>
        </div>
    {:else if (form as { regenerateSecret?: boolean } | null)?.regenerateSecret && newSecret}
        <div class="rounded-xl border border-amber-200 bg-amber-50 p-4">
            ...
            <code class="rounded bg-white px-2 py-1 font-mono text-sm break-all text-gray-800"
                >{newSecret}</code>
        </div>
    {/if}
    ...
    {#if showCreate}
        <div class="rounded-xl border border-blue-100 bg-blue-50 p-5">
            ...
            <form
                ...
                <div>
                    <label for="c-name" class="block text-xs font-medium text-gray-700"
                        >이름 *</label>
                    <input
                        id="c-name"
                        type="text"
                ...
                <div>
                    <label for="c-scopes" class="block text-xs font-medium text-gray-700"
                        >Scopes</label>
                    <input
                        id="c-scopes"
                        type="text"
                ...
                <div class="sm:col-span-2">
                    <label for="c-redirectUris" class="block text-xs font-medium text-gray-700"
                        >Redirect URIs * (줄바꿈 구분)</label>
                    <textarea
                        id="c-redirectUris"
                        name="redirectUris"
                        required
                ...
                <div class="sm:col-span-2">
                    <label for="c-postLogout" class="block text-xs font-medium text-gray-700"
                        >Post-Logout Redirect URIs (줄바꿈 구분)</label>
                    <textarea
                        id="c-postLogout"
                ...
                <div>
                    <label for="c-authMethod" class="block text-xs font-medium text-gray-700"
                        >인증 방식</label>
                    <select
                        id="c-authMethod"
                    ...
                    {#each data.clients as client (client.id)}
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 text-xs text-gray-500"
                                >{client.tokenEndpointAuthMethod}</td>
                            <td class="px-4 py-3">
                                <span
                                    class="rounded-full px-2 py-0.5 text-xs font-medium {client.enabled
                            ...
                            <td class="px-4 py-3 text-xs text-gray-400"
                                >{dateFormatter.format(client.createdAt)}</td>
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    <button
                                        type="button"
                                        onclick={() =>
                                            (editingId =
                                                editingId === client.id ? null : client.id)}
                                    ...
                                    <form method="POST" action="?/regenerateSecret" use:enhance>
                                        ...
                                        <button
                                            ...
                                            onclick={(e) => {
                                                if (
                                                    !confirm(
                                                        "시크릿을 재생성하시겠습니까? 기존 시크릿은 즉시 무효화됩니다.",
                                                    )
                                                )
                                                    e.preventDefault();
                                            }}>
                                    ...
                                    <form
                                        ...
                                        <div class="flex items-center gap-4">
                                            <div class="flex items-center gap-2">
                                                ...
                                                <label
                                                    for="e-pkce-{client.id}"
                                                    class="text-xs text-gray-700">PKCE 필수</label>
                                            </div>
                                            <div class="flex items-center gap-2">
                                                <input
                                                    id="e-enabled-{client.id}"
                                                    type="checkbox"
                                                    name="enabled"
                                                    value="true"
