...
<div class="space-y-6">
	...
	<!-- 생성 폼 -->
	{#if showCreate}
		<div class="rounded-xl border border-blue-100 bg-blue-50 p-5 space-y-4">
			<h2 class="font-semibold text-blue-900">새 SAML SP 등록</h2>

			<!-- 메타데이터 붙여넣기 -->
			<div class="rounded-lg border border-blue-200 bg-white p-4 space-y-2">
				<p class="text-xs font-medium text-gray-700">SP 메타데이터 XML로 자동 입력 <span class="text-gray-400 font-normal">(선택)</span></p>
				{#if metaParseError}
					<div class="rounded border border-red-200 bg-red-50 px-3 py-1.5 text-xs text-red-600">{metaParseError}</div>
				{/if}
				<textarea
			...
			{#if createErr}
				<div class="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">{createErr}</div>
			{/if}
			...
			<form
				...
				use:enhance={() => ({ result, update }) => {
					update();
					if (result.type === 'success') { showCreate = false; resetCreateForm(); }
				}}
				class="grid grid-cols-1 gap-3 sm:grid-cols-2"
			>
				<div>
					<label for="s-name" class="block text-xs font-medium text-gray-700">이름 *</label>
					<input id="s-name" type="text" name="name" required bind:value={createName}
				...
				<div>
					...
					<input id="s-entityId" type="text" name="entityId" required bind:value={createEntityId}
						class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-1.5 text-sm font-mono focus:border-blue-500 focus:outline-none" />
				</div>
				<div class="sm:col-span-2">
					<label for="s-acsUrl" class="block text-xs font-medium text-gray-700">ACS URL *</label>
					<input id="s-acsUrl" type="url" name="acsUrl" required bind:value={createAcsUrl}
						class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-1.5 text-sm focus:border-blue-500 focus:outline-none" />
				</div>
				<div class="sm:col-span-2">
					<label for="s-sloUrl" class="block text-xs font-medium text-gray-700">SLO URL</label>
					<input id="s-sloUrl" type="url" name="sloUrl" bind:value={createSloUrl}
						class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-1.5 text-sm focus:border-blue-500 focus:outline-none" />
				</div>
				<div>
					<label for="s-nameIdFormat" class="block text-xs font-medium text-gray-700">NameID Format</label>
					<select id="s-nameIdFormat" name="nameIdFormat" bind:value={createNameIdFormat}
						class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-1.5 text-sm focus:border-blue-500 focus:outline-none">
				...
				<div class="flex flex-col gap-2 pt-4">
					<div class="flex items-center gap-2">
						<input id="s-signAssertion" type="checkbox" name="signAssertion" value="true"
							bind:checked={createSignAssertion} class="h-4 w-4 rounded border-gray-300" />
						<label for="s-signAssertion" class="text-xs text-gray-700">Assertion 서명</label>
					</div>
					<div class="flex items-center gap-2">
						<input id="s-signResponse" type="checkbox" name="signResponse" value="true"
							bind:checked={createSignResponse} class="h-4 w-4 rounded border-gray-300" />
						<label for="s-signResponse" class="text-xs text-gray-700">Response 서명</label>
					</div>
					<div class="flex items-center gap-2">
						<input id="s-wantSigned" type="checkbox" name="wantAuthnRequestsSigned" value="true"
							bind:checked={createWantSigned} class="h-4 w-4 rounded border-gray-300" />
				...
				<div class="sm:col-span-2">
					...
					<textarea id="s-cert" name="cert" rows="4" bind:value={createCert}
				...
				{:else}
					{#each data.sps as sp (sp.id)}
						<tr class="hover:bg-gray-50">
							...
							<td class="px-4 py-3">
								<div class="flex items-center gap-2">
									<button type="button" onclick={() => (editingId = editingId === sp.id ? null : sp.id)}
								...
								<td colspan="6" class="px-4 py-4">
									...
									>
										...
										<div>
											...
											<input id="e-name-{sp.id}" type="text" name="name" value={sp.name} required
												class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-1.5 text-sm focus:border-blue-500 focus:outline-none" />
										</div>
										<div>
											<label for="e-nameIdFormat-{sp.id}" class="block text-xs font-medium text-gray-700">NameID Format</label>
											<select id="e-nameIdFormat-{sp.id}" name="nameIdFormat"
												class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-1.5 text-sm focus:border-blue-500 focus:outline-none">
												{#each NAME_ID_OPTIONS as opt}
										...
										<div class="sm:col-span-2">
											...
											<input id="e-acsUrl-{sp.id}" type="url" name="acsUrl" value={sp.acsUrl} required
												class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-1.5 text-sm focus:border-blue-500 focus:outline-none" />
										</div>
										<div class="sm:col-span-2">
											<label for="e-sloUrl-{sp.id}" class="block text-xs font-medium text-gray-700">SLO URL</label>
											<input id="e-sloUrl-{sp.id}" type="url" name="sloUrl" value={sp.sloUrl ?? ''}
												class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-1.5 text-sm focus:border-blue-500 focus:outline-none" />
										</div>
										<div class="flex flex-col gap-2">
											<div class="flex items-center gap-2">
												<input id="e-signAssertion-{sp.id}" type="checkbox" name="signAssertion" value="true"
													checked={sp.signAssertion} class="h-4 w-4 rounded border-gray-300" />
												<label for="e-signAssertion-{sp.id}" class="text-xs text-gray-700">Assertion 서명</label>
											</div>
											<div class="flex items-center gap-2">
												<input id="e-signResponse-{sp.id}" type="checkbox" name="signResponse" value="true"
													checked={sp.signResponse} class="h-4 w-4 rounded border-gray-300" />
												<label for="e-signResponse-{sp.id}" class="text-xs text-gray-700">Response 서명</label>
											</div>
											<div class="flex items-center gap-2">
												<input id="e-wantSigned-{sp.id}" type="checkbox" name="wantAuthnRequestsSigned" value="true"
													checked={sp.wantAuthnRequestsSigned} class="h-4 w-4 rounded border-gray-300" />
												<label for="e-wantSigned-{sp.id}" class="text-xs text-gray-700">AuthnRequest 서명 요구</label>
											</div>
											<div class="flex items-center gap-2">
												<input id="e-enabled-{sp.id}" type="checkbox" name="enabled" value="true"
													checked={sp.enabled} class="h-4 w-4 rounded border-gray-300" />
										...
										<div class="sm:col-span-2">
											...
											<textarea id="e-cert-{sp.id}" name="cert" rows="4"
												class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-1.5 font-mono text-xs focus:border-blue-500 focus:outline-none">{sp.cert ?? ''}</textarea>
										</div>
										<div class="sm:col-span-2 flex justify-end gap-2">
											<button type="button" onclick={() => (editingId = null)}
												class="rounded-md border border-gray-300 px-3 py-1.5 text-xs text-gray-600 hover:bg-gray-50">취소</button>
											<button type="submit"
												class="rounded-md bg-blue-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-blue-700">저장</button>
										</div>
									</form>
								</td>
							</tr>
