...
<div class="space-y-6">
	...
	{#if rotated && newKid}
		<div class="rounded-xl border border-green-200 bg-green-50 p-4">
			...
			<p class="text-xs text-green-700">새 KID: <code class="rounded bg-white px-1.5 py-0.5 font-mono">{newKid}</code></p>
		</div>
	{/if}
	...
	<div class="overflow-x-auto rounded-xl border border-gray-200 bg-white shadow-sm">
		<table class="min-w-full divide-y divide-gray-200">
			<thead class="bg-gray-50">
				<tr>
					<th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">KID</th>
					<th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">알고리즘</th>
					<th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">용도</th>
					<th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">인증서</th>
					<th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">상태</th>
					<th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">생성일</th>
					<th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">로테이션</th>
					<th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">만료일</th>
				</tr>
			</thead>
				...
				{:else}
					{#each data.keys as key (key.id)}
						<tr class="hover:bg-gray-50 {key.active ? '' : 'opacity-60'}">
							<td class="px-4 py-3 font-mono text-xs text-gray-700 max-w-[160px] truncate" title={key.kid}>{key.kid}</td>
