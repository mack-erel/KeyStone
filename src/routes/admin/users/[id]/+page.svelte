...
<div class="space-y-8 max-w-3xl">
		...
		<form method="POST" action="?/updateProfile" use:enhance class="space-y-4">
			<div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
				<div>
					...
					<input id="givenName" type="text" name="givenName" value={data.user.givenName ?? ''} class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none" />
				</div>
				<div>
					<label for="familyName" class="block text-xs font-medium text-gray-700">성 (Family Name)</label>
					<input id="familyName" type="text" name="familyName" value={data.user.familyName ?? ''} class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none" />
				</div>
				<div class="sm:col-span-2">
					<label for="displayName" class="block text-xs font-medium text-gray-700">표시 이름</label>
					<input id="displayName" type="text" name="displayName" value={data.user.displayName ?? ''} class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none" />
				</div>
				<div>
					<label for="phoneNumber" class="block text-xs font-medium text-gray-700">전화번호</label>
					<input id="phoneNumber" type="tel" name="phoneNumber" value={data.user.phoneNumber ?? ''} placeholder="+82-10-0000-0000" class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none" />
				</div>
				<div>
					<label for="birthdate" class="block text-xs font-medium text-gray-700">생년월일</label>
					<input id="birthdate" type="date" name="birthdate" value={data.user.birthdate ?? ''} class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none" />
				</div>
				<div class="sm:col-span-2">
					<label for="bio" class="block text-xs font-medium text-gray-700">소개</label>
					<textarea id="bio" name="bio" rows="2" class="mt-1 w-full resize-none rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none">{data.user.bio ?? ''}</textarea>
				</div>
				<div>
	...
	<section class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
		...
		<!-- 부서 추가 폼 -->
		<form method="POST" action="?/addDept" use:enhance class="grid grid-cols-2 gap-2 sm:grid-cols-4 border-t border-gray-100 pt-4">
	...
	<section class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
		...
		<form method="POST" action="?/addTeam" use:enhance class="grid grid-cols-2 gap-2 sm:grid-cols-4 border-t border-gray-100 pt-4">
	...
	<section class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
		...
		<form method="POST" action="?/addPart" use:enhance class="grid grid-cols-2 gap-2 sm:grid-cols-4 border-t border-gray-100 pt-4">
