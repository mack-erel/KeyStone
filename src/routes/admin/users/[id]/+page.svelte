...
<div class="max-w-3xl space-y-8">
    ...
    <div class="flex items-center gap-3">
        <a href={resolve("/admin/users")} class="text-sm text-gray-400 hover:text-gray-600"
            >← 목록</a>
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <select
                        ...
                        {#each LOCALE_OPTIONS as opt (opt.value)}
                            <option value={opt.value} selected={data.user.locale === opt.value}
                                >{opt.label}</option>
                        {/each}
                    </select>
                </div>
                <div>
                    ...
                    <select
                        ...
                        {#each TIMEZONE_OPTIONS as opt (opt.value)}
                            <option value={opt.value} selected={data.user.zoneinfo === opt.value}
                                >{opt.label}</option>
                        {/each}
                    </select>
                </div>
                <div>
                    ...
                    <select
                        ...
                        name="role"
                        class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none">
                        <option value="user" selected={data.user.role === "user"}
                            >일반 사용자</option>
                ...
                <div>
                    ...
                    <select
                        ...
                        name="status"
                        class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none">
                        <option value="active" selected={data.user.status === "active"}
                            >활성</option>
                        <option value="disabled" selected={data.user.status === "disabled"}
                            >비활성</option>
                        <option value="locked" selected={data.user.status === "locked"}
                            >잠김</option>
                    </select>
                </div>
            </div>
    ...
    <section class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                    ...
                    <div class="flex items-center justify-between px-4 py-2.5 text-sm">
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            {#if m.isPrimary}<span
                                    ...
                                    >주소속</span
                                >{/if}
                            {#if m.positionName}<span class="text-gray-500">{m.positionName}</span
                                >{/if}
                            {#if m.jobTitle}<span class="text-gray-400">/ {m.jobTitle}</span>{/if}
                            <span class="text-xs text-gray-300"
                                >{dateFormatter.format(m.startedAt)} ~</span>
                        </div>
        ...
        <form
            ...
            action="?/addDept"
            use:enhance
            class="grid grid-cols-2 gap-2 border-t border-gray-100 pt-4 sm:grid-cols-4">
            <select
                name="departmentId"
                required
            ...
            <div class="flex items-center gap-2">
                ...
                <button
                    type="submit"
                    class="ml-auto rounded-md bg-blue-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-blue-700"
                    >추가</button>
            </div>
        </form>
    </section>
    ...
    <section class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                ...
                {#each data.teamMemberships as m (m.id)}
                    <div class="flex items-center justify-between px-4 py-2.5 text-sm">
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            {#if m.departmentName}<span class="text-xs text-gray-400"
                                    >({m.departmentName})</span
                                >{/if}
                            {#if m.isPrimary}<span
        ...
        <form
            ...
            <select
                name="teamId"
                required
                class="col-span-2 rounded-md border border-gray-300 px-2 py-1.5 text-sm focus:border-blue-500 focus:outline-none sm:col-span-1">
                ...
                {#each data.partMemberships as m (m.id)}
                    <div class="flex items-center justify-between px-4 py-2.5 text-sm">
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            {#if m.teamName}<span class="text-xs text-gray-400">({m.teamName})</span
                                >{/if}
                            {#if m.isPrimary}<span
