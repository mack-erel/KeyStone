    ...
    {#if showCreate}
        <div class="rounded-xl border border-blue-100 bg-blue-50 p-5">
            ...
            <form
                ...
                <div>
                    <label for="new-email" class="block text-xs font-medium text-gray-700"
                        >이메일 *</label>
                    <input
                        id="new-email"
                        type="email"
                ...
                <div>
                    <label for="new-displayName" class="block text-xs font-medium text-gray-700"
                        >이름</label>
                    <input
                        id="new-displayName"
                        type="text"
                ...
                <div>
                    <label for="new-role" class="block text-xs font-medium text-gray-700"
                        >역할 *</label>
                    <select
                        id="new-role"
                        name="role"
                    ...
                    {#each data.users as user (user.id)}
                        <tr class="hover:bg-gray-50">
                                    ...
                                    {#if STATUS_NEXT[user.status]}
                                        <form method="POST" action="?/updateStatus" use:enhance>
                                            ...
                                            <input
                                                type="hidden"
                                                name="status"
                                                value={STATUS_NEXT[user.status].status} />
                                            <button
                                                type="submit"
                            ...
                            <td class="px-4 py-3 text-xs text-gray-400"
                                >{dateFormatter.format(user.createdAt)}</td>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    <button
                                        ...
                                        onclick={() =>
                                            (resetPasswordUserId =
                                                resetPasswordUserId === user.id ? null : user.id)}
