					...
					{#each data.departments as dept (dept.id)}
						<tr class="hover:bg-gray-50">
							...
							{:else}
								...
								<td class="px-4 py-3">
									<span
										class="rounded-full px-2 py-0.5 text-xs font-medium {STATUS_COLOR[dept.status]}"
							...
							<td class="px-4 py-3">
								<div class="flex gap-2">
									<button
										type="button"
										onclick={() => (editId = editId === dept.id ? null : dept.id)}
									...
									<form method="POST" action="?/delete" use:enhance>
										...
										<button
											...
											onclick={(e) => {
												if (!confirm('삭제하시겠습니까? 하위 부서의 상위 부서 연결이 해제됩니다.'))
													e.preventDefault();
											}}>삭제</button
										>
									</form>
