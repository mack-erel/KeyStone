					...
					{#each menuItems as item (item.key)}
						<li>
							<a
								...
								class="block rounded-md px-3 py-2 text-sm font-medium transition-colors {page.url.pathname === resolve(item.route) ? 'bg-blue-50 text-blue-700' : 'text-gray-700 hover:bg-gray-100'}">
								{t(item.key)}
							</a>
						</li>
					{/each}
