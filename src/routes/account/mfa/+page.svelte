		...
		<div class="rounded-2xl border border-gray-200 bg-white p-8 shadow-sm">
			...
			{#if isSetupMode}
				<div class="space-y-5">
					...
					<form method="POST" action="?/confirm" use:enhance class="space-y-3">
						...
						<button type="submit" class="flex w-full justify-center rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white shadow-sm transition hover:bg-blue-700"> 등록 완료 </button>
					</form>
			...
			{:else}
				<div class="space-y-5">
					...
					<form method="POST" action="?/setup" use:enhance>
						<button type="submit" class="flex w-full justify-center rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white shadow-sm transition hover:bg-blue-700"> 인증기 등록 시작 </button>
					</form>
				</div>
			{/if}
		</div>
