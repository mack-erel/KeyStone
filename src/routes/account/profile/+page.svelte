...
<div class="mx-auto max-w-2xl space-y-8 p-6">
    ...
    {#if success}
        <div class="rounded-lg border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-700">{t("profile.saved")}</div>
    {/if}
    {#if err}
    ...
    <form method="POST" use:enhance class="space-y-6">
        <section class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                    <label for="givenName" class="block text-xs font-medium text-gray-700">{t("profile.given_name")}</label>
                    <input
                        id="givenName"
                        type="text"
                        name="givenName"
                ...
                <div>
                    <label for="familyName" class="block text-xs font-medium text-gray-700">{t("profile.family_name")}</label>
                    <input
                        id="familyName"
                        type="text"
                        name="familyName"
                ...
                <div class="sm:col-span-2">
                    <label for="displayName" class="block text-xs font-medium text-gray-700">{t("profile.display_name")}</label>
                    <input
                        id="displayName"
                        type="text"
                        name="displayName"
                ...
                <div>
                    <label for="birthdate" class="block text-xs font-medium text-gray-700">{t("profile.birthdate")}</label>
                    <input
                        id="birthdate"
                        type="date"
                        name="birthdate"
                ...
                <div>
                    <label for="phoneNumber" class="block text-xs font-medium text-gray-700">{t("profile.phone")}</label>
                    <input
                        id="phoneNumber"
                        type="tel"
                        name="phoneNumber"
                ...
                <div class="sm:col-span-2">
                    <label for="bio" class="block text-xs font-medium text-gray-700">{t("profile.bio")}</label>
        ...
        <section class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
            <h2 class="mb-4 text-sm font-semibold text-gray-700">{t("profile.locale_settings")}</h2>
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                    <label for="locale" class="block text-xs font-medium text-gray-700">{t("profile.locale")}</label>
                ...
                <div>
                    <label for="zoneinfo" class="block text-xs font-medium text-gray-700">{t("profile.timezone")}</label>
        ...
        {#if data.membership.departments.length > 0 || data.membership.teams.length > 0 || data.membership.parts.length > 0}
            <section class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                <h2 class="mb-4 text-sm font-semibold text-gray-700">{t("profile.org_membership")}</h2>
                <div class="space-y-3">
                    {#each data.membership.departments as dept (dept.name)}
                        <div class="flex items-center gap-3 rounded-lg bg-gray-50 px-4 py-2.5 text-sm">
                            ...
                            {#if dept.isPrimary}<span class="rounded-full bg-blue-100 px-1.5 py-0.5 text-xs text-blue-700">{t("common.primary_member")}</span>{/if}
                    ...
                    {#each data.membership.teams as team (team.name)}
                        <div class="flex items-center gap-3 rounded-lg bg-gray-50 px-4 py-2.5 text-sm">
                            <span class="text-xs text-gray-500">{t("profile.team_label")}</span>
                            ...
                            {#if team.isPrimary}<span class="rounded-full bg-indigo-100 px-1.5 py-0.5 text-xs text-indigo-700">{t("common.primary_member")}</span>{/if}
                            {#if team.jobTitle}<span class="text-gray-500">/ {team.jobTitle}</span>{/if}
                        </div>
                    {/each}
                    {#each data.membership.parts as part (part.name)}
                        <div class="flex items-center gap-3 rounded-lg bg-gray-50 px-4 py-2.5 text-sm">
                            <span class="text-xs text-gray-500">{t("profile.part_label")}</span>
                            ...
                            {#if part.isPrimary}<span class="rounded-full bg-violet-100 px-1.5 py-0.5 text-xs text-violet-700">{t("common.primary_member")}</span>{/if}
        ...
        <div class="flex justify-end">
            <button type="submit" class="rounded-md bg-blue-600 px-6 py-2 text-sm font-medium text-white hover:bg-blue-700">{t("common.save")}</button>
        </div>
    </form>
</div>
