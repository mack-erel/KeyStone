 ...
 */
export async function ldapSearchDn(
	config: LdapProviderConfig,
	bindDn: string,
	bindPassword: string,
	filter: string
	...
	return new Promise((resolve, reject) => {
		...
		client.bind(bindDn, bindPassword, (bindErr: Error | null) => {
			...
			client.search(
				config.baseDN,
				{ scope: 'sub', filter, attributes: ['dn'] },
				(searchErr: Error | null, res: ldap.SearchCallbackResponse) => {
					if (searchErr) {
						client.unbind();
						reject(searchErr);
						return;
...
export async function ldapFetchEntry(
	config: LdapProviderConfig,
	bindDn: string,
	bindPassword: string,
	entryDn: string,
