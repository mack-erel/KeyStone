 ...
 */
export async function issueEmailVerification(db: DB, userId: string, email: string, platform: App.Platform | undefined): Promise<void> {
    ...
    try {
        ...
        const sendPromise = sendEmailVerificationEmail(email, verifyUrl, platform).catch(() => {
            // 발송 실패는 조용히 무시
        });
